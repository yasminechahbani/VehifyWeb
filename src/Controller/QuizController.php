...
public function edit(Request $request, string $numeroPermis, EntityManagerInterface $entityManager, PermisRepository $permisRepository, QuizRepository $quizRepository): Response
{
    ...
    // When renewing, update the status and reset the renewal flag
    if ($permis->isRenewal()) {
        $permis->setEtat('Active');
        $permis->setIsRenewal(false);
        
        // Update expiration date
        $newExpirationDate = new \DateTime();
        $newExpirationDate->modify('+1 year');
        $permis->setDateExpiration($newExpirationDate);
        
        $entityManager->flush();

        // Send WhatsApp notification
        $employe = $permis->getIdEmploye();
        if ($employe) {
            $message = "Votre permis numéro " . $permis->getNumeroPermis() . " a été renouvelé avec succès. " .
                      "Nouvelle date d'expiration: " . $newExpirationDate->format('d/m/Y');
            
            $this->twilioWhatsappService->sendMessage(
                //$employe->getPhoneNumber(), // Make sure your Employe entity has this getter
                                +2162942403,
                                 // Make sure your Employe entity has this getter

                $message
            );
        }
        
        $this->addFlash('success', 'Permis renouvelé avec succès');
    }
