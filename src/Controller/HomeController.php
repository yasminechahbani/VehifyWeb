...
class HomeController extends AbstractController
{
    ...
    public function index(EmployeRepository $employeRepository,VehiculeRepository $vehiculeRepo, CarteGriseRepository $carteGriseRepo,EquipementRepository $equipementRepository,ReservationRepository $reservationRepo): Response
    {
        ...
        $totalReservations = $reservationRepo->countTotalReservations();
        $activeReservations = $reservationRepo->countActiveReservations();
        $upcomingReservations = $reservationRepo->findUpcomingReservations();
    ...
    public function afficherVehicule(VehiculeRepository $vehiculeRepo, CarteGriseRepository $carteGriseRepo, Request $request, PaginatorInterface $paginator): Response
    {
        // Récupérer la liste des véhicules et des cartes grises
        $queryBuilder = $vehiculeRepo->createQueryBuilder('v');
...
// Méthode de modification pour Véhicule
    ...
    #[Route('/dashboard/search', name: 'app_search', methods: ['GET'])]
    public function search(Request $request, VehiculeRepository $vehiculeRepo, PaginatorInterface $paginator): Response
    {
        $query = $request->query->get('q', '');
        
        $queryBuilder = $vehiculeRepo->createQueryBuilder('v')
            ->leftJoin('v.carteGrise', 'cg')
            ->where('v.marque LIKE :query')
            ->orWhere('v.modele LIKE :query')
            ->orWhere('v.immatriculation LIKE :query')
            ->orWhere('cg.numeroCarteGrise LIKE :query')
