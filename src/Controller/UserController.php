...
final class UserController extends AbstractController
{
    ...
    // Add this method after your class declaration
    private function checkRememberMeCookie(
        Request $request, 
        SessionInterface $session,
        EntityManagerInterface $entityManager
    ): ?Response {
        $rememberMeCookie = $request->cookies->get('remember_me');
        
        if ($rememberMeCookie && !$session->has('id')) {
            $user = $entityManager
                ->getRepository(User::class)
                ->find($rememberMeCookie);
    ...
    // Modify the start of your new() method
    ...
    public function new(
        ...
        $cookieCheck = $this->checkRememberMeCookie($request, $session, $entityManager);
        if ($cookieCheck) {
            return $cookieCheck;
        }
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($imageFile) {
                try {
                    ...
                    $process = new Process([
                        'python',
                        $projectDir . '/scripts/face_detector.py',
                    ...
                    if (!$result['success']) {
                        ...
                        $cloudinary->uploadApi()->destroy(basename($uploadResult['public_id']));
                    ...
                    return $this->render('user/new.html.twig', [
                        ...
                        'error_message' => 'Erreur lors de la vérification de l\'image: ' . $e->getMessage(),
                        'active_panel' => 'signup'
                    ]);
                }
            }
            ...
            if ($email && $password) {
                ...
                $recaptchaResponse = $request->request->get('g-recaptcha-response');
                    ...
                    if ($request->request->get('remember_me')) {
                        ...
                        $session->set('session_lifetime', time() + (30 * 24 * 60 * 60));
                        
                        // Create response with welcome page render
                        $response = $this->render('welcomePage/welcome.html.twig', [
                            'nom' => $user->getNom(),
                            'prenom' => $user->getPrenom(),
                        ]);
                        
                        // Add remember me cookie to the response
                    ...
                    } else if($user->getStatus()==='invalide') {
                    ...
                    {
                        $message = 'Votre compte a été bloqué. Si vous pensez qu il sagit dune erreur, veuillez contacter ladministrateur !';
                    }
                } else {
